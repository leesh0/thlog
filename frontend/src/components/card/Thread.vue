<template>
  ...
  >
    <div id="avatar">
      <a-avatar
        @click="
          $router.push({
            name: 'user',
            params: { user: thread.owner.username },
          })
        "
        size="large"
        class="ring-1 ring-gray-200 cursor-pointer"
        :src="thread.owner.avatar"
      />
      <div class="text-center text-gray-300">
        <a-icon :type="thread.public ? 'unlock' : 'lock'" />
      </div>
    </div>
    <div class="w-full">
      <router-link
      ...
      <div class="text-xs mt-2 items-center flex justify-between">
        <div>
          ...
          <span
            ...
            "
            >{{ thread.owner.nickname }} ・</span
          >
          <span clas="text-gray-500">{{ ago(thread.updated_at) }}</span>
        </div>
    ...
    <div class="flex gap-x-5 items-center text-gray-400 text-sm"></div>
  </div>
</template>
